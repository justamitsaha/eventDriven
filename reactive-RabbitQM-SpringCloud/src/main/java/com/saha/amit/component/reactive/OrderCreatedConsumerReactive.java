...
import com.saha.amit.component.nonReactive.PaymentCreatedPublisherNonReactive;
...
@Component
public class OrderCreatedConsumerReactive {

    private final Receiver receiver;
    private final ObjectMapper objectMapper;
    private final OrderCreatedPublisherReactive orderCreatedPublisherReactive;
    private final PaymentCreatedPublisherNonReactive paymentCreatedPublisherNonReactive;
    private final Logger logger = LoggerFactory.getLogger(OrderCreatedConsumerReactive.class);

    public OrderCreatedConsumerReactive(Receiver receiver,
                                        ObjectMapper objectMapper,
                                        OrderCreatedPublisherReactive orderCreatedPublisherReactive, PaymentCreatedPublisherNonReactive paymentCreatedPublisherNonReactive) {
        this.receiver = receiver;
        this.objectMapper = objectMapper;
        this.orderCreatedPublisherReactive = orderCreatedPublisherReactive;
        this.paymentCreatedPublisherNonReactive = paymentCreatedPublisherNonReactive;
    }
    ...
    private Mono<Void> processDelivery(AcknowledgableDelivery delivery) {
        try {
            ...
            // 👉 Delegate publishing payment.created event via PaymentCreatedEventPublisher
            try {
                paymentCreatedPublisherNonReactive.publishPaymentCreated(order);
                delivery.ack();
            } catch (Exception e) {
                delivery.nack(false);
            }
    ...
    /* Example of using EventPublisher with manual ack/nack handling
       This approach ensures ack is only sent after successful publish.
       Uncomment the call in processDelivery to use this instead of PaymentEventPublisher.
    */
    public Mono<Void> emitPaymentEvent(OrderDto order, AcknowledgableDelivery delivery) {
        // valid: publish payment.created and ack only on success
        return orderCreatedPublisherReactive.publishEvent("payment.created", order)
                .doOnSuccess(v -> {
                    try {
                        delivery.ack();
